...
int main()
{
	...
	try
	{
		...
		if ((hPipe = CreateNamedPipe(
			L"\\\\.\\pipe\\Tube",			//имя кананала (локальный кнанал)
			PIPE_ACCESS_DUPLEX | PIPE_TYPE_BYTE| PIPE_READMODE_BYTE,				//дуплексный канал 
			PIPE_TYPE_MESSAGE | PIPE_READMODE_MESSAGE,  //сообщения|синхронный
			1,								//макс. кол-во экхемпляров канала
			NULL,							//размер выходного буфера
			NULL,							//размер врходного буфера
			NMPWAIT_USE_DEFAULT_WAIT,		//время ожидания связи с клиентом
		...
		//if (!ReadFile(hPipe, ibuf, sizeof(ibuf), &readBytes, NULL))
		...
		for (int i = 0; i < 1001; i++) {
			...
			std::cout<<"Client message:\t\t" << ibuf << std::endl;
			if (!WriteFile(hPipe, ibuf, sizeof(ibuf), &wroteBytes, NULL))
