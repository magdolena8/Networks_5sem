...
bool GetRequestFromClient(char* name, short port, struct sockaddr_in& from, int& flen);
bool  PutAnswerToClient(char* name, struct sockaddr_in& to, int& lto);
...
int main()
{
	...
	try
	{
		...
		if ((sS = socket(AF_INET, SOCK_DGRAM, NULL)) == INVALID_SOCKET)	//создание UDP сокета
...
{
	try {
		from.sin_family = AF_INET;				//использовать IP-адресацию
...
bool  PutAnswerToClient(
	char* name,					//[in] позывной сервера  
	struct sockaddr_in& to,		//[in] указатель на SOCKADDR_IN
	int& lto					//[in] указатель на размер to
)
{
	try
	{
		if ((libuf = sendto(sS, name, sizeof(name), NULL, (sockaddr*)&to, lto)) == SOCKET_ERROR) {
